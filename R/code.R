...
createInstance <- function(
  ...
  if (inherits(with, "data.frame") & !inherits) {
    stop("Data frames are automatically be turned into lists if `inherits = FALSE`\nUnclass input or provide explicit list input")
  }
  cls <- if (inherits)
    unique(c(of, class(with))) else of
...
#' @param attributes \code{\link{list}}
#' Attributes to be passed along to the instance. Supply as a named list.
#' @example inst/examples/create_instance.R
#' @seealso \code{
#'   	\link[instancer]{TODO}
#' }
#' @template authors
